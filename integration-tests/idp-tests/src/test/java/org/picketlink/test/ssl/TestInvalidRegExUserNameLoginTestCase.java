 ...
 * The user is expected not to be authenticated
 * as the regular expression is invalid.
 *
...
@ServerSetup({
    ...
    TestInvalidRegExUserNameLoginTestCase.SecurityDomainsServerSetupTask.class
})
@RunWith(PicketLinkIntegrationTests.class)
@RunAsClient
public class TestInvalidRegExUserNameLoginTestCase extends AbstractIdpSslCertificateLoginTestCase {

    @Rule
    public ExpectedException expextedException = ExpectedException.none();

    @Override
    public void testIdpDeploymentNonSecured(URI uri) throws Exception {
        // Ignore this test
    }

    @Override
    public void testIdpDeployment(URI uri) throws Exception {
        // Ignore this test
    }
    ...
    public void testTrustedCert(@ArquillianResource URI uri) throws Exception {
        ...
        assertThat("Unexpected status code when accessing SP via IdP.", response.getStatusLine().getStatusCode(), equalTo(HttpServletResponse.SC_UNAUTHORIZED));
    }
        ...
        protected SecurityDomain[] getSecurityDomains() throws Exception {
            ...
            res.add(new SecurityDomain.Builder()
              ...
              .loginModules(
                ...
                .flag(Constants.REQUIRED)
                .putOption("password-stacking", "useFirstPass")
                .putOption("regex", "CN=\\(?:[^\",]+|\"[^\"]*\"),")   // This is syntactically invalid regex
                .build(),
